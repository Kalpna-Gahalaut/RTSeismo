function [pcc]=ppstressinduced_p(FNAM,L1,M1,Z1,C1,Tdisc1,SC1,Xobs1,Yobs1,Xdisc1,Ydisc1,nt1,damhmsl1)
            l=L1;
            m=M1;
            z=Z1;
            c=C1;
...
%              pcc(1,:) = [];
            writematrix(pcc,'sample.dat');
	%D5(:,1)=tt;
	%D5(:,2)=pcc
%            pcc(1,:) = [];
%             writematrix(pcc,'data7.dat');
%             ppd=importdata('ppdiff_result_p.dat');
%             data5=importdata('data5.dat');
%             data7=importdata('data7.dat');
%             pps=data5+data7;
%             ppt=ppd+pps;
%             date1=importdata('dateee_p.dat');
%             date2=[date1,ppt,ppd,pps];
%             writematrix(date2,ppt,ppd,pps,'ppstressinduced_result_p.dat');
end
